...
export default function Dashboard() {
  ...
  const handleDelete = async (id: string, type: string) => {
    const confirm = window.confirm(`Delete this ${type}? This will break active Twitter links.`);
    if (!confirm) return;
    ...
    try {
      if (type === "A/B Campaign") {
        ...
        await supabase.from("blinks").delete().eq("id", id);
      }
                          ...
                          <tr className="hover:bg-neutral-800/30 transition-colors group">
                            ...
                            <td className="p-4 text-right">
                              <div className="flex justify-end gap-2">
                                ...
                                <button onClick={() => handleDelete(item.id, item.type)} className="p-2 bg-neutral-800 hover:bg-red-500/20 rounded-lg text-neutral-400 hover:text-red-500 transition-colors opacity-0 group-hover:opacity-100" title="Delete">
